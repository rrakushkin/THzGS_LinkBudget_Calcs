...
% ---- Link budget params ----
sat_tx        = 24.98;   %23 dBm or 315mW which is 24.98 dBm     % [dBm]
geff_satAnt   = 44;        % [dBi]
...
for k = 1:numel(directivity_gsAnt)
    ...
    for j = 1:numel(Elev)
        p_rx_dBm(j) = linkBudget_Simplified(sat_tx, geff_satAnt, directivity_gsAnt(k), freq_Hz, slant_dist_m(j), l_abs(1,1,j)); % result is in dBm
    end
    ...
    name1 = sprintf('%.1f dBi',directivity_gsAnt);

    plot(Elev, linkMargin, 'LineWidth', 1.5,'Color', colors(k,:), ...
        'DisplayName', sprintf('%.2f dBi', directivity_gsAnt(k)));
     if k == numel(directivity_gsAnt)
